import { confirmSuratById } from "@/app/lib/actions";
...
const RequestedConfirmButton = ({ surat }: RequestedConfirmButtonInterface) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [checkBoxNote, setCheckBoxNote] = useState<boolean>(false);
  const [note, setNote] = useState<string>("");
  const verifikasi = async () => {
    const res = await confirmSuratById(surat.id, note)
    if(res) {
